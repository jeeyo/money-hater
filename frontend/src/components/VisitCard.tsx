        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="truncate font-medium text-ink">
              {visit.label}
              {visit.pinned && <Pin className="ml-1 inline size-3.5 text-ink-4" />}
            </p>
            <p className="text-xs text-ink-3">
              {formatTime(visit.started_at)}
              {visit.ended_at !== visit.started_at && <> – {formatTime(visit.ended_at)}</>}
              {visit.place?.formatted_address && (
                <span className="ml-1 inline-flex items-center gap-0.5 text-ink-4">
          ...
          {visit.spend.base_total_minor > 0 && (
            <span className="shrink-0 rounded-full bg-money-bg px-2.5 py-1 text-xs font-semibold text-money">
